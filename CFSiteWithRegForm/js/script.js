...
function addHandlers()
{
	...
	pnoalt[0].addEventListener("blur",checkNumber);
	pnoalt[0].addEventListener("focus",clearErrors);
...
function checkWord()
{
	...
	var value = this;
	$.getJSON('components/fieldValidationService.cfc?method=validateName&fieldvalue=' + value.value , function(result)
	{
...
function checkNumber()
{
	...
	var value = this;
	$.getJSON('components/fieldValidationService.cfc?method=validateNumber&fieldvalue=' + value.value , function(result)
	{
		if (Object.values(result)[0] !== getErrorMessages(value) && Object.values(result)[0] !== "") 
			generateErrors(Object.values(result)[0].toString(), value);
		if (Object.keys(result)[0] !== value)
...
function getErrorMessages(name)
{
	...
	for (iterator = 0; iterator < len; iterator++) 
	{
		if (error[iterator].nextElementSibling === name) {
			return error[iterator].innerHTML;
